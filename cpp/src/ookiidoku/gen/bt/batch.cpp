#include <ookiidoku/gen/bt/batch.hpp>
...
namespace ookiidoku::gen::bt::batch {
	...
	OOKIIDOKU_NO_EXPORT [[gnu::noinline]] BatchReport batch_(const Order O, Params& params, std::function<std::thread(ThreadSharedData&, std::mutex&)> mk_thread) {
		params.clean(O);
		std::mutex sd_mutex;
