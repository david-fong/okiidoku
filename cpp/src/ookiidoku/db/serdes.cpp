...
namespace ookiidoku::db::serdes {
	...
	OOKIIDOKU_EXPORT void print(std::ostream& os, const Order O, const std::span<const T> grid_view) {
	...
	OOKIIDOKU_EXPORT void parse(std::istream& is, const Order O, const std::span<T> grid_view) {
		const Order O4 = O*O*O*O;
		assert(grid_view.size() >= O4);
		const std::streamsize val_size = get_min_bytes_to_store(O*O);
		if (sizeof(T) == val_size) {
