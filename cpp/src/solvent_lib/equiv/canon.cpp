...
namespace solvent::lib::equiv {
	...
	void Canonicalizer<O>::relabel_(void) noexcept {
		...
		of A and B in `grid_` are swapped.
		...
		*/
		...
		// The specific reduction below is a naive joint probability.
		for (ord2_t label = 0; label < O2; label++) {
			static constexpr std::array<double, O2+1> prob = [](){
			...
			const double sort_basis = std::transform_reduce(
				...
				1.0, std::multiplies<double>(), [](const ord2_t count) { return prob[count]; }
			);
		...
		std::ranges::sort(canon2orig_label, std::ranges::less(), &SortMapEntry::sort_basis);
	...
	template<Order O>
	void Canonicalizer<O>::reposition_() {
		const GridSlide grid_slide = GridSlide::build(grid_);
		...
		for (ord2_t canon_row = 0; canon_row < O2; canon_row++) {
			...
			for (ord2_t canon_col = 0; canon_col < O2; canon_col++) {
				...
				// canon_input[orig_row][orig_col] = grid_[canon_row][canon_col];
			}
		}
