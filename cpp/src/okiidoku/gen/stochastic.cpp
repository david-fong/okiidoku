...
namespace okiidoku::gen::ss {
	...
	std::unique_ptr<Generator> Generator::create(const Order order) {
		switch (order) {
			...
			default: return std::make_unique<gen::ss::GeneratorO<M_OKIIDOKU_DEFAULT_ORDER>>();
		}
	}
	...
	template<Order O>
	GeneratorO<O>::val_t GeneratorO<O>::get_val_at_(const GeneratorO<O>::o4x_t coord) const noexcept {
	...
	void GeneratorO<O>::generate_() {
		...
		for (o2i_t h_chute {0}; h_chute < T::O2; h_chute += T::O1) {
			...
			while (has_nots != 0) [[likely]] {
				...
				const auto a_blk {static_cast<o2x_t>(a_col/T::O1)};
				const auto b_blk {static_cast<o2x_t>(b_col/T::O1)};
		...
		for (o2i_t v_chute {0}; v_chute < T::O2; v_chute += T::O1) {
			...
			for (o2i_t blk_col {0}; blk_col < T::O1; ++blk_col) {
