	...
	for (std::uintmax_t round {0u}; round < num_rounds; ++round) {
		...
		const auto written_data {[&]noexcept{
			...
			for (auto i {0uz}; i < num_places; ++i) {
				...
				p.digit = shared_rng() % p.radix;
				if (!writer.accept(p.radix, p.digit)) {
					writer.flush(os);
				}
			}
			return os.str();
		...
		for (auto i {0uz}; i < num_places; ++i) {
			auto& p {places[i]};
			const auto digit {reader.read(is, p.radix, i == num_places-1u)};
