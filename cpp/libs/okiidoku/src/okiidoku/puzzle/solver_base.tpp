#ifndef HPP_OKIIDOKU__PUZZLE__SOLVER_BASE
#define HPP_OKIIDOKU__PUZZLE__SOLVER_BASE
...
namespace okiidoku::mono::detail {
	// TODO.mid consider using type erasure in the solve.hpp header to allow
	// putting impl in

	template<Order O> requires(is_order_compiled(O))
	class SolverBase final {
	public:
		explicit SolverBase(const Grid<O>& puzzle) noexcept;
	...
	private:
		...
		std::queue<QueuedCommit> commit_queue_ {};
		...
		[[nodiscard]] hit_unsat_t solve_fish_() noexcept;
	...
	template<Order O> requires(is_order_compiled(O))
	SolverBase<O>::SolverBase(const Grid<O>& puzzle) noexcept {
	...
	template<Order O> requires(is_order_compiled(O))
	std::optional<Grid<O>> SolverBase<O>::get_next_solution() noexcept {
	...
	template<Order O> requires(is_order_compiled(O))
	typename SolverBase<O>::hit_unsat_t
	SolverBase<O>::remove_cell_candidate_(
		const SolverBase<O>::rmi_t rmi,
		const SolverBase<O>::val_t cand_to_remove
	) noexcept {
	...
	template<Order O> requires(is_order_compiled(O))
	typename SolverBase<O>::hit_unsat_t
	SolverBase<O>::commit_cell_val_(
		const SolverBase<O>::rmi_t rmi,
		const SolverBase<O>::val_t val
	) noexcept {
	...
	template<Order O> requires(is_order_compiled(O))
	void SolverBase<O>::handle_unsat_() noexcept {
