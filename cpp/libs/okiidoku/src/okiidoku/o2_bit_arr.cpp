...
namespace okiidoku::mono {
	...
	auto O2BitArr<O>::count() const noexcept -> typename O2BitArr<O>::o2i_t {
		if constexpr (num_words == 1u) {
			...
			/*RVO*/o2i_t count {std::transform_reduce(OKIIDOKU_UNSEQ
	...
	auto O2BitArr<O>::count_below(const typename Ints<O>::o2x_t end) const noexcept -> typename Ints<O>::o2x_t {
		...
		if constexpr (num_words == 1u) {
			...
			return o2x_t{std::transform_reduce(OKIIDOKU_UNSEQ
	...
	template<Order O> requires(is_order_compiled(O))
	auto O2BitArr<O>::first_set_bit() const noexcept -> typename O2BitArr<O>::o2i_t {
		OKIIDOKU_ASSERT(count() > 0u);
		if constexpr (num_words == 1u) {
			if (words_.front() == 0u) [[unlikely]] { return T::O2; }
			const o2i_t count {std::countr_zero(words_.front())};
			...
			if (word_it == words_.cend()) [[unlikely]] { return T::O2; }
			/*RVO*/o2i_t count {
				(word_t_num_bits * std::distance(words_.cbegin(), word_it))
	...
	auto O2BitArr<O>::to_chars() const noexcept -> std::array<char, Ints<O>::O2> {
		std::array<char, T::O2> _ OKIIDOKU_DEFER_INIT; // NOLINT(*-init)
		_.fill('.');
	...
	#define OKIIDOKU_FOREACH_O_EMIT(O_) \
		...
		template typename O2BitArr<(O_)>::o2i_t O2BitArr<(O_)>::first_set_bit() const noexcept; \
