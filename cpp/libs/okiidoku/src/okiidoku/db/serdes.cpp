...
namespace okiidoku::mono::db::serdes {
	...
	OKIIDOKU_EXPORT void print(std::ostream& os, const GridConstSpan<O> grid_view, const bool is_grid_filled) {
		// const Order O2 = O*O;
		...
		(void)is_grid_filled; (void)os; (void)grid_view;
	...
	OKIIDOKU_EXPORT void parse(std::istream& is, const GridSpan<O> grid_view, const bool is_grid_filled) {
		...
		(void)is_grid_filled; (void)is; (void)grid_view;
	...
	#define OKIIDOKU_FOR_COMPILED_O(O_) \
		template void print<O_>(std::ostream&, GridConstSpan<O_>, bool); \
